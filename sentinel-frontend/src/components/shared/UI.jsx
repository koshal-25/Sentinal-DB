...
// ── Table ─────────────────────────────────────────────────────
export function Table({ cols, columns, rows, data, onRow, onRowClick, emptyMsg = 'No records found.' }) {
  const actualCols = cols || (columns ? columns.map(c => ({ label: c, key: c })) : []);
  const actualRows = rows || data || [];
  const handleRowClick = onRow || onRowClick;
  const isArrayData = data !== undefined;
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-gray-800">
            {actualCols.map((c, idx) => (
              <th key={c.key || c.label || idx}
        ...
        <tbody>
          {actualRows.length === 0 ? (
            <tr><td colSpan={actualCols.length} className="px-3 py-8 text-center text-gray-600 text-xs">{emptyMsg}</td></tr>
          ) : actualRows.map((row, i) => (
            <tr key={i}
                onClick={() => handleRowClick ? handleRowClick(isArrayData ? i : row) : null}
                className={`border-b border-gray-800/50 hover:bg-gray-800/40 transition-colors ${handleRowClick ? 'cursor-pointer' : ''}`}>
              {isArrayData ? (
                row.map((cell, j) => (
                  <td key={j} className="px-3 py-2.5 text-gray-300">{cell ?? '—'}</td>
                ))
              ) : (
                actualCols.map((c, j) => (
                  <td key={c.key || c.label || j} className="px-3 py-2.5 text-gray-300">
